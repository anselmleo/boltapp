...
<template>
  <header>
    <div class="absolute top-0 left-0 right-0 container flex justify-between w-8/12 mx-auto p-5">
      <RouterLink to="/">
        <svg
          width="54"
          height="32"
      ...
      <nav class="h-10">
        <div class="flex" :class="pageTheme.textColor">
          ...
          <div class="w-20 rounded-full border-current mr-10" :class="pageTheme.borderColor">
            <RouterLink
              to="/signup"
              class="pl-3.5 font-bold text-sm"
              :class="pageTheme.signupTextColor"
              >Sign up</RouterLink
            >
          </div>
          <img
  ...
  <!-- <RouterView class="absolute text-cyan-400 bg-cyan-100" /> -->
</template>
